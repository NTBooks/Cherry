...
function addSway(mat, strength) {
  mat.onBeforeCompile = (sh) => {
    ...
    sh.vertexShader = ('uniform float uTime; uniform float uSway; uniform vec2 uPush;\n' + sh.vertexShader).replace(
      ...
      transformed += vec3(sin(sph), sin(sph * 0.83 + 1.7) * 0.6, cos(sph * 0.91)) * uSway;
      transformed += vec3(sin(sph * 2.63 + 1.3), sin(sph * 2.21 + 0.6) * 0.5, cos(sph * 2.4 + 2.1)) * uSway * 0.5;
      transformed.xz += uPush * (0.6 + 0.4 * sin(sph * 0.7));`,
    );
  };
}
...
const leafCardTex = canvasTex(256, (g, s) => {
  ...
  for (let i = 0; i < 95; i++) {
    ...
    const rx = rand(9, 15), ry = rand(5, 8), rot = rng() * Math.PI;
...
function makeTwigTextures() {
  ...
  const stroke = (x0, y0, x1, y1, w) => {
    g1.strokeStyle = 'rgba(70,54,42,1)';
    g1.lineWidth = w;
  ...
  // Limbs taper along their run and drift gently upward like a cherry's sweep.
  const branch = (x, y, ang, len, w, depth) => {
    ...
    for (let k = 0; k < segs; k++) {
      ...
      stroke(px, py, nx, ny, Math.max(1, w * (1 - k / (segs + 1))));
      if (depth < 2 && k >= 1) {
        const side = rng2() < 0.5 ? 1 : -1;
        branch(nx, ny, a + side * rand2(0.45, 0.85), len * rand2(0.5, 0.65), w * 0.55, depth + 1);
      }
      px = nx; py = ny;
    }
    if (depth < 2) branch(px, py, a + rand2(-0.3, 0.3), len * 0.4, w * 0.45, depth + 1);
  };
  for (let b = 0; b < 4; b++) {
    branch(128, 134, UP + rand2(-1.6, 1.6), rand2(80, 112), rand2(6.5, 8.5), 0);
  }
...
const twigMat = new THREE.MeshToonMaterial({
  map: twigCardTex, gradientMap: toonRamp, alphaTest: 0.5, side: THREE.DoubleSide, color: 0x6e6058,
});
const twigSnowMat = new THREE.MeshToonMaterial({
  map: twigSnowTex, gradientMap: toonRampSoft, alphaTest: 0.04, transparent: true, opacity: 0,
...
const twigDepthMat = new THREE.MeshDepthMaterial({
  depthPacking: THREE.RGBADepthPacking, map: twigCardTex, alphaTest: 0.5,
});
...
const skyMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    uniform float uCloudAmt, uStar, uTime, uMoonVis, uDusk, uOvercast;
...
function buildTree(x, z, treeIdx) {
  ...
  anchors.push([top.clone().add(new THREE.Vector3(0, 0.3, 0)), rand(1.25, 1.5)]);
  anchors.push([top.clone().add(new THREE.Vector3(rand(-0.5, 0.5), 0.15, rand(-0.5, 0.5))), rand(1.0, 1.3)]);
  ...
  for (const [c, size] of anchors) {
    for (let k = 0; k < 3; k++) {
      ...
      dummy.position.set(c.x + rand(-0.12, 0.12), c.y + rand(-0.08, 0.12), c.z + rand(-0.12, 0.12));
  ...
  // the permanent twig lattice (slightly inflated clones so nothing is coplanar)
  const twigMesh = new THREE.Mesh(cardGeo.clone().scale(1.012, 1.012, 1.012), twigMat);
  twigMesh.castShadow = true;
  twigMesh.customDepthMaterial = twigDepthMat;
  grp.add(twigMesh);
  const snowTwigMesh = new THREE.Mesh(cardGeo.clone().scale(1.024, 1.024, 1.024), twigSnowMat);
...
class DriftingInstances {
  constructor({ count, geometry, material, fall, sway, spin, colors }) {
    this.count = count;
  ...
  update(dt, t, weight, spawnFn) {
    ...
    const target = Math.round(this.count * Math.min(1, weight * 1.2));
    let toSpawn = Math.min(Math.max(0, target - active), Math.ceil(this.count * dt * 0.5) + 1);
    let dirty = false;
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      if (it.landT > 0) {
        it.landT += dt;
        const shrink = 1 - THREE.MathUtils.smoothstep(it.landT, 5, 6.6);
        this.writeMatrix(i, it.size * shrink);
        if (it.landT > 6.6) { it.active = false; this.writeMatrix(i, 0); }
        continue;
      }
      it.pos.y -= it.vy * dt;
...
const spawnPetal = makeCanopySpawner('covB');
...
function adjustSpeed(d) {
  ...
  speedEl.textContent = `time ${timeSpeed > 0 ? '+' : ''}${timeSpeed}`;
...
function animate() {
  ...
  if (canvas.width !== Math.floor(window.innerWidth * renderer.getPixelRatio()) ||
      canvas.height !== Math.floor(window.innerHeight * renderer.getPixelRatio())) {
  ...
  for (const tree of trees) {
    ...
    // carpets: pink after the petal storm, russet after leaf-fall, buried in winter
    tmpColor.copy(CARPET_SPRING_C).multiplyScalar(carpetSpringW);
  ...
  petals.update(sdt, simT, petalStorm(seasonT) + 0.3 * blossomCoverage(seasonT), spawnPetal);
  ...
  puddleW += (rainW - puddleW) * Math.min(1, sdt / (rainW > puddleW ? 7 : 28));
  const pVis = puddleW > 0.02;
  for (const p of puddles) p.visible = pVis;
